...
#The total number of unique movies and users in the “edx” subset is ;
edx %>%
...
#When we explore the distributions of the ratings, it is seen that users prefer to rate movies rather higher than lower as shown by the graphics below. 4 is the most common rating, followed by 3 and 5. 0.5 is the least common rating. 
...
#As seen in the graphics showing “MovieId vs number of ratings” and “number of ratings vs. number of movies” below, some movies have been rated much often than other, while some have very few ratings and even some have only one rating. 
...
#We can see that the majority of users have rated between 30 and 100 movies. 
...
#The expected rating of the underlying dataset is;
# Dataset's average rating

avg_ratg <- mean(edx$rating)
avg_ratg

#If we predict all unknown ratings with µ or mu, we obtain the first naive RMSE:
...
# Plot penalty term user effect (We compute the average rating for user µ, for those that have rated over 100 movies, said penalty term user effect. )#
...
#We compute an approximation by computing µ and bi, and estimating ue, as the average of "(Yu,i − µ − bi)".
...
#Estimates of bi and ue are caused by movies with very few ratings and in some users that only rated a very small number of movies. Hence this can strongly influence the prediction. The use of the regularization permits to penalize these aspects. We should find the value of lambda (that is a tuning parameter) that will minimize the RMSE. This shrinks the bi and ue in case of small number of ratings.
...
#We find the lowest value of RMSE that is "0.8648170".
rmse_results %>% knitr::kable()
